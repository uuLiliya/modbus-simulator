...
/*
 * MBAP Header（Modbus Application Protocol Header，共7字节）
 *
 * // 事务标识符(2字节) | 协议标识符(2字节) | 长度(2字节) | 单元标识符(1字节)
 * // 0x00 0x01        | 0x00 0x00        | 0x00 0x06   | 0x01
 *
 * 事务标识符用于匹配请求和响应；协议标识符固定为0x0000；
 * 长度字段表示后续 Unit ID + PDU 的字节数；单元标识符用于路由到目标设备。
 */
typedef struct {
 ...
 * // 0x03         | ...
 *
 * 功能码定义执行的操作类型（如0x03 读保持寄存器、0x06 写单个寄存器），
 * 后续数据字段根据功能码不同而变化。
 */
typedef struct {
...
/*
 * FC03 读保持寄存器 - 请求格式（5字节 PDU）
 *
 * // 功能码(1字节) | 起始地址高字节 | 起始地址低字节 | 寄存器数量高字节 | 寄存器数量低字节
 * // 0x03         | 0x00          | 0x6B          | 0x00             | 0x03
 *
 * 起始地址和寄存器数量采用大端序编码，数量范围为 1-125。
 */
typedef struct {
...
/*
 * FC03 读保持寄存器 - 响应格式（可变长度 PDU）
 *
 * // 功能码(1字节) | 字节计数(1字节) | 寄存器高字节 | 寄存器低字节 | 寄存器高字节 | 寄存器低字节 | 寄存器高字节 | 寄存器低字节
 * // 0x03         | 0x06           | 0x02        | 0x2B        | 0x00        | 0x00        | 0x00        | 0x64
 *
 * 字节计数等于寄存器数量 * 2，每个寄存器以大端序传输高低字节。
 */
typedef struct {
...
/*
 * FC06 写单个寄存器 - 请求格式（5字节 PDU）
 *
 * // 功能码(1字节) | 寄存器地址高字节 | 寄存器地址低字节 | 寄存器值高字节 | 寄存器值低字节
 * // 0x06         | 0x00            | 0x01            | 0x00          | 0x03
 *
 * 寄存器地址和值均使用大端序编码，响应会原样回显该结构。
 */
typedef struct {
